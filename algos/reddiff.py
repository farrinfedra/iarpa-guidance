...
@register_module(category="algo", name="reddiff")
class REDDIFF(DDIM):
    ...
    def sample(self, x, y, ts, **kwargs):
        ...
        for ti, si in zip(reversed(ts), reversed(ss)):
            ...
            et = self.model(xt, y, t, scale=scale)  # et, x0_pred
            x0_hat = self.diffusion.predict_x_from_eps(xt, et, t)
            ...
            snr_inv = (1 - alpha_t[0]).sqrt() / alpha_t[
                0
            ].sqrt()  # 1d torch tensor #noise to signal ratio
