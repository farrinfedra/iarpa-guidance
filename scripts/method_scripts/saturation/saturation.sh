exp_root=./iarpa_results #the root and the exp_name.
ROOT_DIR=$(pwd)
ckpt_root=/extra/ucibdl1/shared/projects/oc-guidance/fmaroufs/ckpts
save_deg=True
save_ori=True
overwrite=True
smoke_test=200 # Controls the number of batches generated
batch_size=5

## Dataset and Model Params ##
config_name=imagenet256_uncond #dataset
model=imagenet256_uncond
...
### Fixed params ###
difusion_steps=50
M=2  # Num u_t optimization steps
eta=0.1  # DDIM stochasticity parameter (0 for deterministic sampling)
t_start=0

samples_root=\"best_lpips_${algo}_${model}_u_lr=${u_lr}_init_xT=${algo_init_xT}_t_start=${t_start}_t_end=${t_end}_gamma_t=${combine_fn_gamma_t}_w_terminal=${w_terminal}_w_control=${w_control}_w_score=${w_score}_${deg_file}${factor}\"
